...
def unzip_data_extract_contents():
    ...
    if all(
        [
            ...
            _unsup_data,
            _imdb_vocab,
            _imdb_expected_rating,
            _train_matrix is not None,
            _test_matrix is not None,
            _unsup_matrix is not None,
        ]
    ):
        train_archive = Archive(
            type="train", labeled_bow=_train_matrix, reviews=_train_data.get("reviews")
        )
        test_archive = Archive(
            type="test", labeled_bow=_test_matrix, reviews=_test_data.get("reviews")
        )
        unsup_archive = Archive(
            type="unsup", labeled_bow=_unsup_matrix, reviews=_unsup_data.get("reviews")
        )
    ...
    train_archive.add_labeled_bow(
        bow_dicts_to_matrix(train_bow_contents, len(imdb_vocab))
    )
    test_archive.add_labeled_bow(
        bow_dicts_to_matrix(test_bow_contents, len(imdb_vocab))
    )
    unsup_archive.add_labeled_bow(
        bow_dicts_to_matrix(unsup_bow_contents, len(imdb_vocab))
    )
